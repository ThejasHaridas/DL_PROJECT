...
class Tetris:
    ...
    def move_down(self):
        """Moves the current piece one position down if possible."""
    ...
    def move_piece(self, action):
        """Execute a piece movement based on action."""
        x, rotation = action
        ...
        # Then move horizontally to target position
        while current_x < x:
    ...
    def play(self, x, rotation, render=False, render_delay=None):
        ...
        # Drop piece
        if render:
            final_pos = self.move_piece((x, rotation))
            if render_delay:
                sleep(render_delay)
            self.render()
        else:
